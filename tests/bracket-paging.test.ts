...
describe("buildBracketTabs", () => {
  ...
  it("moves cross-page rounds into the finals tab", () => {
    ...
    const finals = tabs.find((entry) => entry.key === "FINALS");
    expect(finals).toBeDefined();
    expect(finals?.pages[0]?.rounds[0]?.matches[0]?.id).toBe("WINNERS-2-1");
  });
